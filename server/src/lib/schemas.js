/**
 * Server-side action payload validation (zod) — port persis api/_lib/schemas.js.
 * Mirrors critical schemas dari src/lib/schemas.ts. Tetap sinkron saat
...
const ACTION_SCHEMAS = {
  ...
  listLetters: z.object({
    action: z.literal('listLetters'),
    eventId: z.string().optional(),
    draftEventId: z.string().optional(),
  }),
  createLetter: z.object({
    action: z.literal('createLetter'),
    eventId: z.string().nullish(),
    draftEventId: z.string().nullish(),
    letterData: z.record(z.string(), z.unknown()),
    pdfBase64: z.string().max(2_500_000).nullish(),
    pdfUrl: z.string().nullish(),
    createdBy: z.string().nullish(),
  }),
  updateLetter: z.object({
    action: z.literal('updateLetter'),
    id: z.string().min(1),
    updates: z
      .object({
        letterData: z.record(z.string(), z.unknown()).optional(),
        pdfUrl: z.string().nullish(),
        pdfBase64: z.string().max(2_500_000).nullish(),
