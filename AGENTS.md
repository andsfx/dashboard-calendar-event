...
Auth: login POST `/auth/login` (bcrypt compare `users.password_hash` — akun Supabase-Auth lama di-reset via `server/scripts/create-admin.mjs --reset`) → HttpOnly cookies `sb-access-token`/`sb-refresh-token` (JWT HS256 `jose`, payload `{sub,role,email}`; **COOKIE_SAMESITE=none + Secure** wajib di produksi karena SPA Vercel cross-site). Role hierarchy `superadmin > admin > demo > viewer > eo_tenant | tenant_relation`; matriks per-role di `server/src/auth.js` + `server/src/routes/tenant.js`. Role `demo` = lihat semua, ubah tidak — ditegakkan backend (`DEMO_READ_ACTIONS` allowlist di `server/src/auth.js`; aksi di luar allowlist ditolak 403), jadi menyembunyikan tombol di UI saja tidak cukup. `usePermission` (`src/hooks/usePermission.ts`) maps roles ke granular booleans; flag `canView*` memisahkan "boleh lihat" dari `canManage*` ("boleh kelola"). Login legacy Apps Script / `admin_session` = mati.
...
- `src/pages/` — komponen level-route (public + `/dashboard` login), tiap file 1 rute lazy; `src/components/` — semua **berkelompok**, tidak ada file flat di akarnya: `modals/` (dialog/overlay), `views/` (mode tampilan: kalender, kanban, timeline), `events/` (widget domain event), `drafts/` (antrian pra-jadwal), `media/` (galeri/lightbox), `ui/` (primitif bersama + `index.ts` barrel), `shared/`, `nav/`, plus feature folder `dashboard/`, `community/`, `survey/`, `admin/`, `forms/`, `pdf/`. Test colocated di `__tests__/` milik folder masing-masing
...
- `e2e/` — Playwright specs + `helpers.ts`; `docs/` — `SPEC.md`, `docs/adr/` (termasuk `005-vps-postgres-lepas-supabase.md`), `docs/agents/`, `docs/tickets/`, `docs/features/` (brief `update-fitur-*.md`), `docs/research/`, `docs/reports/`
...
- **Kolom non-skalar wajib lewat `server/src/lib/pgValues.js`** — `events.categories`/`draft_events.categories` bertipe **`TEXT[]`** (bukan JSONB): kirim **array JS** via `toTextArray(...)`, JANGAN `JSON.stringify(...)` — itu menghasilkan `malformed array literal` (22P02) dan muncul ke user sebagai "Gagal memperbarui — Perubahan belum tersimpan". Hanya `day_time_slots` yang JSONB (`toJsonb`/`JSON.stringify`). Berlaku di `admin.js` (createEvent/updateEvent/batchCreateEvents/publishDraft/createDraft/updateDraft) dan `public.js` (`POST /drafts`).
...
- Workflows: GitHub issues via `gh` CLI on `andsfx/dashboard-calendar-event` with the five-label triage vocabulary (`needs-triage`, `needs-info`, `ready-for-agent`, `ready-for-human`, `wontfix`) — see `docs/agents/`; read `CONTEXT.md` + `docs/adr/` before exploring domain questions; feature work follows the doc-first `update-fitur-*.md` brief pattern in `docs/features/`. CRUD diverifikasi langsung pada produksi (Vercel SPA + API `api.metmalcommunityspace.web.id`), tidak hanya lokal; jangan jalankan resource-destructive checks ke produksi (read-only verification).
