...
- **Navigasi header halaman komunitas (`/`) dikelompokkan jadi dropdown kategori.** Sebelumnya 11 link dideretkan dalam satu baris horizontal (`NAV_ITEMS`) — melebar dan padat di layar sempit. Kini 4 entri top-level: `Event` (link langsung `#upcoming-events`), `Program ▾` (`#benefits` Keuntungan · `#areas` Area & Fasilitas · `#how` Cara Daftar · `#faq` FAQ), `Jelajahi ▾` (`#gallery` Galeri · `#news` Berita · `/tenants` Tenant · `/pameran` Pameran), dan `Kontak` (link langsung `#contact`). Tombol `Daftar Sekarang` dan `Jadwal Event` tidak disentuh (memang di luar `NAV_ENTRIES`).
  - **Komponen baru `src/components/nav/NavDropdown.tsx`** — trigger + panel yang dikonfigurasi lewat array bertipe (`NavDropdownItem[]`), sehingga `CommunityLandingPage` tetap deklaratif. Pola ARIA menu-button: trigger `aria-haspopup="menu"` + `aria-expanded`, panel `role="menu"` dengan item `role="menuitem"`. Buka via hover (mouse, dengan intent delay 120 ms) **dan** klik/Enter/Space; tutup via Escape (fokus balik ke trigger), klik di luar, kehilangan fokus (Tab keluar), atau setelah item dipilih. `ArrowDown`/`ArrowUp` masuk & bersiklus di daftar item; hanya satu dropdown terbuka pada satu waktu (state `openMenu` di parent); fokus tidak pernah terkunci. Anchor `#…` tetap scroll in-page, `/tenants` & `/pameran` tetap navigasi react-router `<Link>` (`route: true`).
  ...
  - **Test baru `src/components/__tests__/NavDropdown.test.tsx`** (7 kasus): buka via klik + `aria-expanded` bertukar, tutup via Escape dengan fokus kembali ke trigger, navigasi `ArrowDown`/`ArrowUp` + wrap, item route `/tenants` & `/pameran` ter-render sebagai link, tutup via klik di luar, dan hanya satu dropdown terbuka. Semua id section (`#benefits`, `#areas`, `#how`, `#faq`, `#gallery`, `#news`, `#contact`, `#upcoming-events`, `#register`, `#hero`) tidak berubah.
