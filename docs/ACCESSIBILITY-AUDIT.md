...
- Smoke berjalan pada **dev login-shell** (`VITE_DEV_AUTO_LOGIN` → superadmin dev) + data publik via REST publik/js (read-only). Endpoint mutasi admin tidak dievaluasi (dev tanpa backend/tanpa proxy `:3001`) → state mutasi admin nyata tidak tercakup.
