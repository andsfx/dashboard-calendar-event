...
| D7 | Tipe: `EventArea` + `AreaPhoto` di `src/types/index.ts` (pola `PhotoAlbum`); mapper `dbEventAreaToEventArea` / `areaPhotoToDbRow` di `albumsApi.ts` (boundary snake_case↔camelCase). |
