...
## 7. Tipe — `src/types/index.ts` TIDAK BERUBAH

Shape direktori = `TenantRosterItem { id; name; floor; lot; category; logo }` yang **sudah ada** di `src/utils/api/surveysApi.ts` (L399) dan sudah dipakai publik (`results-roster`). Tidak perlu tipe baru di `src/types/index.ts` — hindari duplikasi (pola jenis tipe API-scoped di `surveysApi.ts`).
...
- `TenantRosterItem` (`src/utils/api/surveysApi.ts` L399) — reuse tipe; tidak tambah di `src/types/index.ts`.
