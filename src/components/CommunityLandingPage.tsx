...
/* ─── Event Showcase (4 views) ───────────────────────────── */

const EVENT_VIEW_TABS = [
  { key: 'featured' as const, label: 'Featured', icon: <Sparkles className="h-3.5 w-3.5" /> },
  { key: 'calendar' as const, label: 'Kalender', icon: <CalendarDays className="h-3.5 w-3.5" /> },
  { key: 'timeline' as const, label: 'Timeline', icon: <Clock3 className="h-3.5 w-3.5" /> },
  { key: 'grid' as const, label: 'Grid', icon: <Users className="h-3.5 w-3.5" /> },
];

type EventViewMode = 'featured' | 'calendar' | 'timeline' | 'grid';

const MONTH_NAMES_SHORT = ['Jan','Feb','Mar','Apr','Mei','Jun','Jul','Agu','Sep','Okt','Nov','Des'];
const DAY_NAMES_SHORT = ['Min','Sen','Sel','Rab','Kam','Jum','Sab'];

// ── View 1: Featured Cards (horizontal scroll) ──

function FeaturedCardsView({ events, onDetail }: { events: EventItem[]; onDetail: (ev: EventItem) => void }) {
  ...
  return (
    <div className="mt-8 flex gap-5 overflow-x-auto pb-4 snap-x snap-mandatory scrollbar-thin lg:grid lg:grid-cols-3 lg:overflow-visible lg:pb-0">
      {sorted.map(ev => {
        ...
        return (
          <button
            ...
            type="button"
            onClick={() => onDetail(ev)}
            className="group min-w-[280px] snap-start cursor-pointer overflow-hidden rounded-2xl border border-black/[0.06] bg-white text-left shadow-sm transition hover:shadow-lg hover:-translate-y-1 dark:border-slate-700 dark:bg-slate-800 lg:min-w-0"
          >
            {/* Gradient header */}
            <div
              className="relative px-5 pb-4 pt-5"
              style={{ background: `linear-gradient(135deg, ${color}22 0%, ${color}08 100%)`, borderBottom: `2px solid ${color}33` }}
            >
              <div className="absolute top-0 left-0 right-0 h-1" style={{ background: `linear-gradient(90deg, ${color}, ${color}66)` }} />
              {isOngoing ? (
                <span className="inline-flex items-center gap-1.5 rounded-full bg-emerald-100 px-2.5 py-1 text-[11px] font-bold text-emerald-700 dark:bg-emerald-900/40 dark:text-emerald-300">
                  <Radio className="h-3 w-3 animate-pulse" /> BERLANGSUNG
                </span>
              ) : (
                <span className="inline-flex items-center gap-1.5 rounded-full bg-amber-100 px-2.5 py-1 text-[11px] font-bold text-amber-700 dark:bg-amber-900/40 dark:text-amber-300">
                  <Clock3 className="h-3 w-3" /> {ev.day}, {ev.tanggal}
                </span>
              )}
              <p className="mt-3 text-base font-bold leading-snug text-slate-900 dark:text-white line-clamp-2">{ev.acara}</p>
            </div>
            {/* Body */}
            <div className="px-5 py-4">
              <div className="space-y-2 text-xs text-slate-500 dark:text-slate-400">
                {isOngoing && <p className="font-medium text-slate-700 dark:text-slate-200">{ev.day}, {ev.tanggal}</p>}
              ...
              {ev.keterangan && <p className="mt-2.5 line-clamp-2 text-xs text-slate-400 leading-relaxed">{ev.keterangan}</p>}
...
// ── View 2: Mini Calendar ──

function MiniCalendarView({ events, onDetail }: { events: EventItem[]; onDetail: (ev: EventItem) => void }) {
  ...
  const listTitle = selectedDate
    ? (() => { const d = new Date(year, month, parseInt(selectedDate.split('-')[2])); return `${DAY_NAMES_SHORT[d.getDay()]}, ${parseInt(selectedDate.split('-')[2])} ${MONTH_NAMES_SHORT[month]}`; })()
    : `Event ${MONTH_NAMES_SHORT[month]} ${year}`;
  ...
  return (
    <div className="mt-8 flex flex-col gap-6 lg:flex-row lg:items-start">
      {/* Calendar grid */}
      <div className="overflow-hidden rounded-2xl border border-black/[0.06] bg-white shadow-sm dark:border-slate-700 dark:bg-slate-800 lg:w-80 lg:shrink-0">
        <div className="flex items-center justify-between bg-gradient-to-r from-violet-600 to-indigo-600 px-4 py-3">
          <button onClick={prevMonth} className="rounded-lg p-1.5 text-white/80 transition hover:bg-white/20 hover:text-white"><ChevronDown className="h-4 w-4 rotate-90" /></button>
          <div className="text-center">
            <p className="text-sm font-bold text-white">{MONTH_NAMES_SHORT[month]} {year}</p>
            <p className="text-[10px] text-white/60">{monthEvents.length} event</p>
          </div>
          <button onClick={nextMonth} className="rounded-lg p-1.5 text-white/80 transition hover:bg-white/20 hover:text-white"><ChevronDown className="h-4 w-4 -rotate-90" /></button>
        </div>
        <div className="grid grid-cols-7 border-b border-slate-100 dark:border-slate-700">
          {DAY_NAMES_SHORT.map(d => <div key={d} className="py-1.5 text-center text-[10px] font-semibold text-slate-400">{d}</div>)}
        </div>
        <div className="grid grid-cols-7 gap-0.5 p-1.5">
          {days.map((d, i) => {
            if (!d) return <div key={`e-${i}`} />;
            const hasEvents = !!eventsByDate[d.dateStr]?.length;
            ...
            return (
              <button
                ...
                className={`relative flex h-8 w-full items-center justify-center rounded-lg text-[11px] font-medium transition ${
                  isSelected ? 'bg-violet-600 text-white shadow' : isToday ? 'bg-violet-100 text-violet-700 font-bold ring-1 ring-violet-300 dark:bg-violet-900/40 dark:text-violet-300' : 'text-slate-700 hover:bg-slate-100 dark:text-slate-300 dark:hover:bg-slate-700'
                }`}
              >
                {d.day}
                {hasEvents && <span className={`absolute bottom-0.5 h-1 w-1 rounded-full ${isSelected ? 'bg-white' : 'bg-violet-500 dark:bg-violet-400'}`} />}
              </button>
            );
          })}
        </div>
      ...
      <div className="flex-1">
        <p className="mb-3 text-sm font-bold text-slate-800 dark:text-white">{listTitle} ({selectedEvents.length})</p>
        {selectedEvents.length === 0 ? (
          <EmptyEvents />
        ) : (
          <div className="space-y-2.5 max-h-[400px] overflow-y-auto pr-1">
            {selectedEvents.slice(0, 6).map(ev => (
              <button key={ev.id} type="button" onClick={() => onDetail(ev)} className="w-full cursor-pointer rounded-xl border border-black/[0.06] bg-white p-4 text-left shadow-sm transition hover:shadow-md hover:-translate-y-0.5 dark:border-slate-700 dark:bg-slate-800">
                <div className="flex items-start justify-between gap-3">
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-semibold text-slate-800 dark:text-white line-clamp-1">{ev.acara}</p>
                    <div className="mt-1.5 flex flex-wrap items-center gap-3 text-xs text-slate-500 dark:text-slate-400">
                      <span className="font-medium text-slate-700 dark:text-slate-200">{ev.day}, {ev.tanggal}</span>
                      {ev.jam && <span className="flex items-center gap-1"><Clock className="h-3 w-3" />{ev.jam}</span>}
                      {ev.lokasi && <span className="flex items-center gap-1"><MapPin className="h-3 w-3" /><span className="line-clamp-1">{ev.lokasi}</span></span>}
                    </div>
                  </div>
                  {ev.status === 'ongoing' && (
                    <span className="shrink-0 inline-flex items-center gap-1 rounded-full bg-emerald-100 px-2 py-0.5 text-[10px] font-bold text-emerald-700 dark:bg-emerald-900/40 dark:text-emerald-300">
...
function TimelineEventsView({ events, onDetail }: { events: EventItem[]; onDetail: (ev: EventItem) => void }) {
  const sorted = [...events].sort((a, b) => {
    if (a.status === 'ongoing' && b.status !== 'ongoing') return -1;
    if (a.status !== 'ongoing' && b.status === 'ongoing') return 1;
    return a.dateStr.localeCompare(b.dateStr);
  ...
  return (
    <div className="relative mt-8 ml-4 space-y-6 border-l-2 border-slate-200 pl-6 dark:border-slate-700 sm:ml-6 sm:pl-8">
      {sorted.map((ev, idx) => {
        const color = CATEGORY_COLORS[ev.category] ?? '#6366f1';
        const isOngoing = ev.status === 'ongoing';
        const isToday = ev.dateStr === todayStr;
        const dotColor = isOngoing ? 'bg-emerald-500 ring-emerald-200 dark:ring-emerald-800' : 'bg-amber-500 ring-amber-200 dark:ring-amber-800';

        return (
          <div key={ev.id} className="relative">
            {/* Timeline dot */}
            <div className={`absolute -left-[31px] top-4 h-3.5 w-3.5 rounded-full ring-4 sm:-left-[37px] ${dotColor} ${isOngoing ? 'animate-pulse' : ''}`} />

            {/* Date label */}
            <p className="mb-2 text-xs font-bold text-slate-500 dark:text-slate-400">
              {isToday ? 'Hari Ini' : `${ev.day}, ${ev.tanggal}`}
              {isOngoing && <span className="ml-2 inline-flex items-center gap-1 rounded-full bg-emerald-100 px-2 py-0.5 text-[10px] font-bold text-emerald-700 dark:bg-emerald-900/40 dark:text-emerald-300"><Radio className="h-2.5 w-2.5" /> LIVE</span>}
            </p>
            ...
            <button
              type="button"
              onClick={() => onDetail(ev)}
              className="w-full cursor-pointer overflow-hidden rounded-xl border border-l-4 bg-white p-4 text-left shadow-sm transition hover:shadow-md hover:-translate-y-0.5 dark:bg-slate-800"
              style={{ borderLeftColor: color, borderColor: undefined }}
            >
              <p className="text-sm font-bold text-slate-800 dark:text-white line-clamp-2">{ev.acara}</p>
              <div className="mt-2 flex flex-wrap items-center gap-3 text-xs text-slate-500 dark:text-slate-400">
                {ev.jam && <span className="flex items-center gap-1"><Clock className="h-3 w-3" />{ev.jam}</span>}
                {ev.lokasi && <span className="flex items-center gap-1"><MapPin className="h-3 w-3" /><span className="line-clamp-1">{ev.lokasi}</span></span>}
              </div>
              {ev.keterangan && <p className="mt-2 line-clamp-2 text-xs text-slate-400 leading-relaxed">{ev.keterangan}</p>}
...
function EventShowcase({ events, onDetail, onViewAll }: { events: EventItem[]; onDetail: (ev: EventItem) => void; onViewAll: () => void }) {
  const [view, setView] = useState<EventViewMode>('featured');

  return (
    <>
      {/* View switcher */}
      <div className="mt-8 flex flex-wrap justify-center gap-2">
        {EVENT_VIEW_TABS.map(tab => (
          <button
            key={tab.key}
            type="button"
            ...
            className={`inline-flex items-center gap-1.5 rounded-full px-4 py-2 text-xs font-semibold transition ${
              view === tab.key
                ...
                : 'border border-black/[0.06] bg-white text-slate-600 hover:bg-slate-50 dark:border-slate-700 dark:bg-slate-800 dark:text-slate-300 dark:hover:bg-slate-700'
            }`}
          >
            {tab.icon}
            {tab.label}
      ...
      {/* Content */}
      {view === 'featured' && <FeaturedCardsView events={events} onDetail={onDetail} />}
      {view === 'calendar' && <MiniCalendarView events={events} onDetail={onDetail} />}
      {view === 'timeline' && <TimelineEventsView events={events} onDetail={onDetail} />}
      {view === 'grid' && <GridCardsView events={events} onDetail={onDetail} />}
